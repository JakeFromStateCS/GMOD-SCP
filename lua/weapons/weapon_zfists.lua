...
function SWEP:Initialize()
	...
	self.NextSoundTick = CurTime() + self.SoundDelay;
end
...
function SWEP:SecondaryAttack()
	if( CurTime() > self.NextSoundTick ) then
		self.Owner:EmitSound( SCP.Classes[49].ZombieSounds.Idle .. math.random( 1, 16 ) .. ".wav" );
		self.NextSoundTick = CurTime() + self.SoundDelay;
	end;
end
...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) ) then
		if( self.Owner:InPocketDimension() and tr.Entity:InPocketDimension() or !self.Owner:InPocketDimension() and !tr.Entity:InPocketDimension() ) then
